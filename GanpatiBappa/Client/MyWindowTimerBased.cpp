...
void CALLBACK TimerProcOne(HWND hwnd, UINT msg, UINT_PTR eventMsg, DWORD ms) {
	...
	if (syncVariable == 1) {
		...
		if (i > ContourCount/2 && j != PointsArray[i] - 2){
...
void CALLBACK TimerProcTwo(HWND hwnd, UINT msg, UINT_PTR eventMsg, DWORD ms) {
	...
	if (syncVariable == 1) {
		...
		if (k > 0 && m != PointsArray[k] - 2) {

			if (m < PointsArray[k] - 1) {

				Ellipse(hdc, DrawPoints[k][m] - 2, DrawPoints[k][m + 1] - 2, DrawPoints[k][m] + 2, DrawPoints[k][m + 1] + 2);
				m += 2;
			}
			else
				m = 0;
